...
{{--                                </a>--}}
                                ...
                                <a href="{{ !empty($jobApplication->profile) ?($profiles . $jobApplication->profile) : $logo."avatar.png" }}" class=" avatar-sm">
                    ...
                    <dl class="row">
                        ...
                        <dd class="col-sm-9">
                            {{-- @if(!empty($jobApplication->resume))
                                ...
                                <a href="{{asset(Storage::url('uploads/job/resume')).'/'.$jobApplication->resume}}" download="" class="btn btn-sm bg-primary"  target="_blank"><i class="ti ti-download text-white"></i></a>
                            </span>
                            @else
                                -
                            @endif --}}
                            <a href="{{ route('job.application.user.cv', $jobApplication->user_id) }}"  data-title="{{__('CV / Resume')}}" class="btn-sm btn btn-primary">
                                {{__('CV / Resume')}}
                            </a>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            @endif
            {{Form::open(array('route'=>array('job.application.skill.store',$jobApplication->id),'method'=>'post', 'class'=>'needs-validation', 'novalidate'))}}
            ...
            {{Form::open(array('route'=>array('job.application.note.store',$jobApplication->id),'method'=>'post', 'class'=>'needs-validation', 'novalidate'))}}
            <div class="form-group">
                <label class="form-label">{{__('Applicant Notes')}}</label><x-required></x-required>
                <textarea name="note" class="form-control" id="" rows="3" required placeholder="{{ __('Applicant Notes') }}"></textarea>
            </div>
